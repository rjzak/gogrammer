...
/*
Taken from https://github.com/wangjohn/quickselect
Modified for UInt32 type, and largest not smallest values.
...
type UIntSlice32 []uint32

func (t UIntSlice32) Len() int {
	return len(t)
}

func (t UIntSlice32) Less(i, j int) bool {
	return t[i] > t[j] // Swapped for greater, since we're using UInt32
}

func (t UIntSlice32) Swap(i, j int) {
	t[i], t[j] = t[j], t[i]
}

type UIntArraySlice32 [][]uint32

func (t UIntArraySlice32) Len() int {
	return len(t)
}

func (t UIntArraySlice32) Less(i, j int) bool {
	return t[i][1] > t[j][1] // Swapped for greater, since we're using UInt32; each element is {ngramHash, counts}, and we care about counts
}

func (t UIntArraySlice32) Swap(i, j int) {
	t[i], t[j] = t[j], t[i]
}

func (t UIntArraySlice32) QuickSelect(k int) error {
...
func (t UIntSlice32) QuickSelect(k int) error {
	return QuickSelect(t, k)
}
...
func UInt32QuickSelect(data []uint32, k int) error {
	return QuickSelect(UIntSlice32(data), k)
}
